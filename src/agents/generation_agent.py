...
class GenerationAgent(BaseAgent):
    ...
    def ask_structure(self, info: dict, structure: str) -> str:
        ...
        sys_prompt = "请根据上述给出的目标和题目结构，命制一道高中数学题。你需要给出题目的具体题面，其中公式使用latex格式。如果是简答题，则需要在题面中使用（1）、（2）等来标识各小问，如果是选择填空题，则只需命制一道题。请注意，你给出的题目应该确保题目难度符合给出的题目结构中的难度要求，并且随小问逐级递增。此外，题面中请尽量不要包含图像信息，如果一定要包含图像，请尽可能详细地描述这个图像。最后，在回答中只需要给出题目，不需要包含题目的答案或其他额外表述。"
